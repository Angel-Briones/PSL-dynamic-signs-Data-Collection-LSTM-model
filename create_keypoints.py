...
def create_keypoints(frames_path, save_path):
    '''
    ### CREATE KEYPOINTS FOR A WORD
    Go through the word frames folder and save its keypoints in `save_path`
    '''
    data = pd.DataFrame([])
...
if __name__ == "__main__":
    ...
    with warnings.catch_warnings():
        ...
        # GENERATE THE KEYPOINTS OF ALL THE WORDS
        ...
        # GENERATE THE KEYPOINTS OF ONLY A WORD
        # word_name = "hola"
        # word_path = os.path.join(words_path, word_name)
        # hdf_path = os.path.join(data_path, f"{word_name}.h5")
        # create_keypoints(word_path, hdf_path)
