...
def get_keypoints(model, path):
    '''
    ### OBTAIN SAMPLE'S KEYPOINTS
    Returns the sequence of sample keypoints
    '''
    kp_seq = np.array([])
...
def insert_keypoints_sequence(df, n_sample: int, kp_seq):
    '''
    ### INSERT THE SAMPLE'S KEYPOINTS INTO THE DATAFRAME
    Returns the same DataFrame but with the sample keypoints added
    '''
